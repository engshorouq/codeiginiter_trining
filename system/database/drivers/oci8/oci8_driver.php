...
class CI_DB_oci8_driver extends CI_DB {
	...
	protected function _execute($sql)
	{
		...
		//oci_set_prefetch($this->stmt_id, 1000);
   		...
   		return @oci_execute($this->stmt_id,$this->_commit);//, $this->_commit);
	}
	...
	{
		if (($package == '' && $procedure == '') OR ! is_array($params))
		{
			if ($this->db_debug)
			{
	...
	private function _bind_params($params)
	{
		...
		foreach ($params as $param)
		{
			...
			oci_bind_by_name($this->stmt_id, $param['name'], $param['value'], ($param['length'])?$param['length']:NULL, ($param['type'])?$param['type']:NULL);
